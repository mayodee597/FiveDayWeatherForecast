...
$("#search").on("click", function(event) {
  ...
  .then(function(response) {
    ...
    var uvQuery = `https://api.openweathermap.org/data/2.5/uvi?appid=${key}&lat=${response.city.coord.lat}&lon=${response.city.coord.lon}`;
  ...
  $("#city-submit").on("click", function(event) {
    ...
    for (var i = 0; i<response.list.length; i++) {
     ...
     //dateTime[0] = date, dateTime[1] = time
      ...
      var url = "https://openweathermap.org/img/w/" + response.list[i].weather[0].icon + ".png";
